...
public class OrderServiceImpl implements OrderService {
    ...
    public void submit(String name, String phone, String addr, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (user == null)
            throw new LoginException("请登录！");
        User loginUser = (User) user;
        Order order = new Order();        //new一个订单对象
        order.setName(name);
        order.setPhone(phone);
        order.setAddr(addr);
        order.setOrderTime(new Date());
        order.setUserId(loginUser.getId());
        order.setState(STATE_NO_PAY); //未支付      //设置各项订单属性
        List<OrderItem> orderItems = shopCartService.listCart(request); //得到该用户购物车里的条款项   orderItems为一个数组
        Double total = 0.0;
        order.setTotal(total);
        order = orderDao.save(order);//订单保存数据库
